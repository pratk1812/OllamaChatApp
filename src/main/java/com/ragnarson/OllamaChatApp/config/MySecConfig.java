package com.ragnarson.OllamaChatApp.config;
...
public class MySecConfig {
    ...
    http.sessionManagement(
            session ->
                session
                    ...
                    .sessionConcurrency(
                        concurrency ->
                            concurrency.maximumSessions(1).maxSessionsPreventsLogin(true))
                    .sessionFixation()
                    .changeSessionId())
        .headers(
            headers ->
        ...
        .authorizeHttpRequests(
            authorize ->
                authorize
                    ...
                    .permitAll()
                    .requestMatchers(
                        RegexRequestMatcher.regexMatcher("/(addStudent|readStudent)/[A-Za-z0-9]+"))
                    .hasAuthority(Roles.USER.name())
                    .requestMatchers(
                        RegexRequestMatcher.regexMatcher(
                            "/(deleteStudent|updateStudent)/[A-Za-z0-9]+"))
                    .hasAuthority(Roles.ADMIN.name())
                    .requestMatchers(
                        "/chat/**", "/app/**", "/topic/**", "/queue/**", "/gs-guide-websocket")
                    .authenticated()
                    .requestMatchers("/")
                    .permitAll()
                    .anyRequest()
  ...
  public AuthenticationManager authenticationManager(HttpSecurity httpSecurity) {
    return httpSecurity.getSharedObject(AuthenticationManagerBuilder.class).getOrBuild();
  }
}
